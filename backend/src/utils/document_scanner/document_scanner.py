...
def load_model(num_classes=2, model_name="mbv3", device=torch.device("cpu")):
    ...
    if model_name == "mbv3":
        ...
        checkpoint_path = os.path.join(os.getcwd(), "document-scanner/model_mbv3_iou_mix_2C_aux.pth")
    else:
...
def scan(image_path: str, trained_model: DeepLabV3, image_size=384, BUFFER=10, threshold=11, c=1):
    """
    ...
    # Read image
    image_true = cv2.imread(image_path)
